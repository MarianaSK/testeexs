...
//====================================================================================
  ...
  void AvailableMessage()
  {
    ...
    for(uint8_t i = 0; i < MAXSC; i++)
    {
      if (ESPClient[i] && ESPClient[i].connected() && ESPClient[i].available())
      {
          ...
          {
            Message[0] = ESPClient[0].readStringUntil('\r');
            Message[1] = ESPClient[1].readStringUntil('\r');
            ESPClient[i].flush();
            ESPClient[i].read();
            //float(Message);    
            ClientNumber();
            Serial.println("Client No " + String(i+1) + " - " + "Temperatura 1: " + Message[0] + "Temperatura 2: " + Message[1]); 
            temp1=Message[0].toFloat();
            temp2=Message[1].toFloat();
 ...
 // =================================================================================
   ...
   void  ClientNumber(){
      ...
      if (Message[0] == "<Cliente 01-1>"){                           // Verifica o número do cliente, 01 = cliente 1, e depois do traço, estado 1 = on, 0 = off
             digitalWrite(LED2, HIGH);
      }
       if(Message[0] == "<Cliente 01-0>"){
              digitalWrite(LED2, LOW);
            }
       if (Message[1] == "<Cliente 02-1>"){
              digitalWrite(LED3, HIGH);
      }
       if(Message[1] == "<Cliente 02-0>"){
