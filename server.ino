...
void onoff_led(){
  if ((hour() == 11) && (minute() >= 0) && (minute() <= 2) && (weekday() >= 2) && (weekday() <= 6)){
  ...
  if((hour() == 11) && (minute() > 2) && (minute() <= 4) && (weekday() >= 2) && (weekday() <= 6)){
...
//====================================================================================
 ...
 // ESTRUTURA DE DADOS
  void tabelaCliente(){
    ...
    //Cliente 1 testa a condicao de temperatura inválida
    /*segundoPostadoC1=poscliente[1][4].toInt();
    //segundoPassadoC1=poscliente[1][4].toInt();
    if(!(temperature[0]>=15) && !(temperature[0]<=31)){
      Serial.println("teste******");   
      for(segundoPassadoC1; segundoPassadoC1<tempoPermitido; segundoPassadoC1++){
        CalcAverage();
        if(temperature[0]>=15 && temperature[0]<=31){
          CalcAverage();
        }
          Serial.print("Segundo em que o cliente caiu: ");
          Serial.println(segundoPostadoC1);
          Serial.print("Segundos passados desde que o cliente caiu: ");
          Serial.println(segundoPassadoC1);
          delay(1000);
        if(segundoPassadoC1>tempoPermitido){
          medirTemperaturaUmidade();
        }
      }
        //allNewClientsFound = false;
    ...
    //Cliente 2 testa a condicao de cliente desconectado
...
/*bool dadosValidos(){
  segundoPostadoC1=poscliente[1][4].toInt();
    //segundoPassadoC1=poscliente[1][4].toInt();
    while((!(temperature[0]>=15 && temperature[0]<=31))||(!ESPClient[0])){
      Serial.println("teste******");
      for(segundoPassadoC1; segundoPassadoC1<10; segundoPassadoC1++){
         if((temperature[0]>=15 && temperature[0]<=31)||(ESPClient[0])){
          CalcAverage();
        }else{
          medirTemperaturaUmidade();
         }
